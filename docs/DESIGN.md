---
version: alpha
name: QA-Console-design-system
description: A confident professional interface anchored on a clean white canvas, where black (#111111) carries every primary CTA and brand moment. Type runs Inter / IBM Plex Sans / JetBrains Mono stack at modest weights — the system trusts size and black-white contrast over bold weight. All surfaces use the light theme with black CTAs and gray hairlines. Status colors green (pass) and red (fail) accents thread through for validation signals.
...
typography:
  hero-display:
    fontFamily: "IBM Plex Sans, Inter, Noto Sans SC, -apple-system, BlinkMacSystemFont, sans-serif"
    fontSize: 64px
    fontWeight: 700
    lineHeight: 1.1
    letterSpacing: -1px
  display-lg:
    fontFamily: "IBM Plex Sans, Inter, Noto Sans SC, sans-serif"
    fontSize: 48px
    fontWeight: 700
    lineHeight: 1.1
    letterSpacing: -0.5px
...
QA Console is a professional testing platform interface that feels both authoritative and efficient. The base atmosphere is **clean white canvas** (`{colors.canvas}` — #ffffff) holding dark text and a single, ubiquitous accent: **Black** (`{colors.primary}` — #111111). That black does almost all of the brand's heavy lifting — it carries every primary CTA, every section headline, every featured action, and the wordmark itself. There is no secondary brand color. The system trusts the black-white contrast to do the brand work, and it carries it.

Type runs **Inter** (body + UI), **IBM Plex Sans** (display + titles), and **JetBrains Mono** (numerical / technical data) stack. IBM Plex Sans carries display headlines and section titles. Inter handles body copy and UI elements. JetBrains Mono appears on ticket numbers, test point counts, numerical stats — anywhere a number or code reference wants to feel "precise and reliable." All run at modest weights — display sizes use weight 600-700, body stays at 400.

The product is **light-theme only**: all surfaces default to white canvas with gray card backgrounds. The same black CTAs and gray hairlines (`{colors.hairline}` — #eaecef) thread throughout. Status **green** (`{colors.pass}` — #0ecb81) and **red** (`{colors.fail}` — #f6465d) signal validation results in tables, badges, and status indicators across all surfaces.

**Key Characteristics:**
- Single primary color: `{colors.primary}` (#111111) does all brand emphasis — primary CTAs, hero headlines, brand mark. Used consistently across all surfaces.
- Modern type stack: `Inter` (body + UI), `IBM Plex Sans` (display), and `JetBrains Mono` (numbers, tickets, technical data). Numbers always render in JetBrains Mono for technical consistency.
- Light theme only: white canvas (`{colors.canvas}`) with gray cards (`{colors.surface-card}`). Black CTAs and status colors are shared across all surfaces.
- Status semantics: green pass / red fail (`{colors.pass}` / `{colors.fail}`) for validation states, applied as text color or badge background.
- Card surfaces: `{colors.surface-card}` (#fafafa) for elevated cards. No gradient surfaces, no atmospheric backdrops — flat color blocks throughout.
- Border radius is small: `{rounded.md}` (6px) for primary buttons, `{rounded.lg}` (8px) for inputs and content cards, `{rounded.xl}` (12px) for elevated card containers.
- Spacing follows a 4-multiple scale; major sections sit at `{spacing.section}` (80px) — slightly tighter than typical marketing-only sites because product pages need denser layouts.
